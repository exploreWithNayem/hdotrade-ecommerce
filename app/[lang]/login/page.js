import SocialMediaLogin from "@/components/auth/SocialMediaLogin";
import LoginForm from "@/components/auth/login/LoginForm";
import { getLang } from "@/languages/dynamicLangSwitch";
import Link from "next/link";

export default async function page(props) {
  const params = await props.params;

  const {
    lang
  } = params;

  const lan = await getLang(lang);
  return (
    <>
      <div className="contain py-16">
        <div className="max-w-lg mx-auto shadow px-6 py-7 rounded overflow-hidden">
          <h2 className="text-2xl uppercase font-medium mb-1">
            {lan?.logins?.title}
          </h2>
          <p className="text-gray-600 mb-6 text-sm">{lan?.logins?.subtitle}</p>
          <LoginForm langCode={lang} lan={lan?.logins} />

          <SocialMediaLogin lan={lan?.logins} />

          <p className="mt-4 text-center text-gray-600">
            {lan?.logins?.notHaveAccount}{" "}
            <Link href={`/${lang}/register`} className="text-primary">
              {lan?.logins?.register}
            </Link>
          </p>
        </div>
      </div>
    </>
  );
}
